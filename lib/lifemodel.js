...
export function buildLifeModel({ name, age, status, mentions = [], knownAge, knownSex }, now = Date.now()) {
  const year = new Date(now).getFullYear();
  const sex = knownSex || inferSex(name);
  ...
  // Age priority: Wikidata (knownAge) > parsed from text > assumed default.
  let startAge;
  let ageKnown;
  let ageSource;
  if (knownAge != null) {
  ...
  return {
    ...
    ageSource,
    confidence:
      ageSource === "wikidata" ? (sex ? "HIGH" : "MED") : ageSource === "text" ? "MED" : "LOW",
  };
}
